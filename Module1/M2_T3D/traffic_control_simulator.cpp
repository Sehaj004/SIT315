...
void* produceTrafficData(void* args)
{
...
void* consumeTrafficData(void* args)
{
...
void readTrafficData()
{
    ...
    string filename;
    cout << "Greetings! From Anshpreet Singh. Welcome to Traffic Control" << endl;
...
int main()
{
    ...
    for(long i=0; i<numProducers; i++) pthread_create(&producerThreads[i], NULL, produceTrafficData, (void*) i);
    for(long i=0; i<numConsumers; i++) pthread_create(&consumerThreads[i], NULL, consumeTrafficData, (void*) i);
